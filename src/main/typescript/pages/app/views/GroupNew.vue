          ...
          <validation-observer v-slot="{ invalid }">
            <validation-provider v-slot="{ errors, valid }" name="Название" rules="required|max:128">
              <b-form-group label="Название:">
                ...
                <b-form-invalid-feedback v-if="errors.length > 0 && (!valid || valid == null)" :state="valid">
...
export default class extends Vue {
  ...
  created() {
    this.members.push({
      id: this.appStore.getters.account.id,
      name: this.appStore.getters.account.name,
      role: "OWNER"
    });
  }
  ...
  private async createGroup(): Promise<void> {
    const newGroupId = await api.createGroup({
      name: this.name,
      members: this.members.filter(m => m.id != this.appStore.getters.account.id)
    });
    if (newGroupId != null) {
